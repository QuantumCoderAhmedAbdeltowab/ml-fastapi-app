...
class FeatureExtraction:
    def __init__(self, url):
        ...
        # initial fetch + parse (we’ll still do a fresh call in http())
        try:
            self.response = requests.get(url, timeout=10)
            self.soup = BeautifulSoup(self.response.text, 'html.parser')
        except Exception:
    ...
    def http(self):
        """
        Returns:
          1  if the site exists and returns any 2xx status code
         -1  if unreachable or returns non‑2xx
        """
        try:
    ...
    def HTTPS(self):
        try:
            return 1 if self.urlparse_obj.scheme == 'https' else -1
        except Exception:
            return -1
    ...
    def ServerFormHandler(self):
        try:
            ...
            for f in forms:
                ...
                if a in ("", "about:blank"):
                    return -1
                if self.url not in a and self.domain not in a:
                    return 0
            return 1
        except Exception:
            return -1
